...
from db_commands import search_disc_user, search_osu_user, insert_user, find_map

LEGAL_MODS = ["EZ","HD","HR","SD","DT","HT","NC","FL","SO","PF","CL","NF"]

def find_illegal_mod(score):
    contains_cl = False
    print("starting loop")
...
async def illegal_mod_detection(score, interaction: discord.Interaction):
...
class Submit(commands.Cog):
    ...
    async def submit(self, interaction: discord.Interaction, score_id: int):
        ...
        user_db_row = search_disc_user(interaction.user.id)
        if not user_db_row:
        ...
        if await asyncio.to_thread(find_map, score.beatmap_id):
            ...
            if score.pp:
                await interaction.response.send_message("Ok now i need to implement adding the score :)")
                return
            else:
